...
1. A **"Propaganda Index"** score — a quantitative assessment of how closely the content matches known Russian propaganda patterns and active-measure narratives, grounded where possible in Mitrokhin active measures material and later Russian information operations.

The focus is purely on **what the text says and how it says it**:
...
**Objective:** Create canonical ontologies:
...
**Objective:** Use an LLM to pre-label propaganda patterns and TTPs, then correct/curate manually.
...
- [ ] **AI-Assisted Labeling Script** (`scripts/ai_label_bootstrap.py`)
  - Reads from `data/raw/propaganda/`
  - Calls LLM with appropriate prompt
  - Writes candidate labels to `data/labeled_candidates/propaganda/`

- [ ] **Human Review Workflow**
  - You (and any collaborator) review AI labels
  - Use CSV/Label Studio for quick correction
  - Save final labels to `data/labeled_reviewed/propaganda/`
...
## Phase 2: Build Sentence-Level Classifiers
...
### 2.2 Train Propaganda Pattern Classifier
...
## Phase 3: Build the Propaganda Index Models
...
### 3.3 Train Propaganda Index Models
...
- [ ] **Pipeline Wrapper** (`src/index_pipeline.py`)
  - Steps:
    ...
    2. Run propaganda classifier → collect predictions
    3. Aggregate to features for each index
    4. Run index models → get numeric scores
    5. Call explainer (below) → textual justification
  - Method: `analyze(text) → {propaganda_index, explanations, per_sentence_labels}`
